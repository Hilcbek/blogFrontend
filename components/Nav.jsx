...
const Nav = () => {
    ...
    let { profile,username } = useSelector((state) => state.user);
  ...
  return (
    <nav
      ...
      } fixed mb-10 bg-white w-full xs:px-2 xl:px-10 flex items-center justify-between py-2`}
    >
      ...
      <ul className="flex items-center justify-center">
        {reload.reload && profile && (
          <button
            ...
            className="mr-2 p-3 flex items-center justify-center rounded-3xl shadow-lg text-xs font-semibold"
          >
            <BsFillFilePostFill className="mr-2" /> Add Blog
          </button>
        )}
        <div
          onClick={handleShow}
          className="p-2 z-[99] rounded-3xl shadow-md flex items-center justify-center cursor-pointer relative"
        >
          <BiMenu className="mr-1 text-xl" />
          <div className="w-8 h-8 ml-1 border-solid border-gray-500 border-[1px] rounded-full">
            <img
              src={reload.reload && profile ? profile : "/logo.png"}
              alt="none"
              className="w-full h-full object-cover rounded-full"
            />
          </div>
          {username && (
            <h1 className="ml-2 text-xs font-bold">Hello👋 {username}</h1>
          )}
          {reload.reload && profile ? (
            <div
              className={`${
              ...
              } transition duration-300 transform absolute rounded-md top-14 p-2 w-40 right-2 shadow-md bg-white flex items-start justify-start flex-col`}
            >
              <Link
                to={"/ownPost"}
                className={`${
                  show ? "flex" : "hidden"
                } w-full hover:bg-gray-50 p-2 items-center justify-start cursor-pointer font-medium text-left text-xs rounded-sm`}
              >
                <GrBlog size={19} className="mr-1" /> My Blogs
              </Link>
              <MenuItem
                status={show}
                Icon={<BiLogOut size={19} />}
                label={"Logout"}
            ...
            <div
              className={`${
                show
                  ? "translate-y-0 opacity-100 z-[9999]"
                  : "-translate-y-[10px] -z-[9999] opacity-0"
              } transition duration-300 transform absolute rounded-md top-14 p-2 w-40 right-2 shadow-md bg-white flex items-start justify-start flex-col`}
            >
              <MenuItem
                status={show}
